...
static RFC_3339_24_TIME_REGEX: LazyLock<Regex> = LazyLock::new(|| {
...
pub fn validate_rfc3339_24hr_time(value: &str) -> Result<(), OcppError> {
    if RFC_3339_24_TIME_REGEX.is_match(value) {
        ...
        Err(OcppError::FieldISOError { value: value.to_string(), iso: "8601 (RFC-3339)".to_string() })
    }
}
    ...
    fn test_edge_case_dates() {
        ...
        for (date, should_be_valid) in edge_cases {
            ...
            if should_be_valid {
                ...
                println!("Note: '{}' passes regex but may be invalid calendar date", date);
            }
        }
    }
